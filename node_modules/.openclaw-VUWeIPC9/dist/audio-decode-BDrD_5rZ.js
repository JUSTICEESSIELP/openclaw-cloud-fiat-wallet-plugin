...
const decoders = {
	async oga(buf) {
		...
		if (!decoder) {
			let { OggVorbisDecoder } = await import("./ogg-vorbis-DBWvZlnU.js");
	...
	async flac(buf) {
		...
		if (!decoder) {
			const { FLACDecoder } = await import("./flac-DXUA7X_o.js");
	...
	async opus(buf) {
		...
		if (!decoder) {
			const { OggOpusDecoder } = await import("./ogg-opus-decoder-D1sn0hhe.js");
	...
	async wav(buf) {
		...
		if (!decode) {
			let module = await import("./node-wav-Bf_z-HQO.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1));
	...
	async qoa(buf) {
		...
		if (!decode) decoders.qoa.decode = decode = (await import("./qoa-format-BCGxG-uo.js")).decode;
		return buf && createBuffer(await decode(buf));
	}
};
