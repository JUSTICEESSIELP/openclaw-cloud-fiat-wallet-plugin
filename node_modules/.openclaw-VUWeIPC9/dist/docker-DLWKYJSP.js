import { o as resolveRequiredHomeDir, s as resolveRequiredOsHomeDir } from "./home-dir-BnP38vVl.js";
...
import "./agent-scope-C9FCuuJ_.js";
...
import { t as acquireSessionWriteLock } from "./session-write-lock-C36UApB6.js";
import { r as writeJsonAtomic } from "./json-files--_DxMfNw.js";
import { t as safeParseJsonWithSchema } from "./zod-parse-8Ohapmo_.js";
import { d as DEFAULT_SANDBOX_IMAGE, f as SANDBOX_AGENT_WORKSPACE_MOUNT, h as SANDBOX_REGISTRY_PATH, p as SANDBOX_BROWSER_REGISTRY_PATH } from "./config-C-FN-elV.js";
import { t as sanitizeEnvVars } from "./sanitize-env-vars-BWxI_PTN.js";
import { i as resolveWindowsSpawnProgram, n as materializeWindowsSpawnProgram } from "./windows-spawn-BMCppuAI.js";
...
function getBlockedReasonForSourcePath(sourceNormalized) {
	...
	for (const blocked of getBlockedHostPaths()) if (sourceNormalized === blocked || sourceNormalized.startsWith(blocked + "/")) return {
		kind: "targets",
		blockedPath: blocked
	};
	return null;
...
function getBlockedHostPaths() {
	...
	const aliases = [];
	for (const candidate of BLOCKED_HOST_PATHS) aliases.push(resolveBlockedHostPathAlias(candidate));
	for (const home of new Set([effectiveHome, osHome])) {
		if (home === "/") continue;
		for (const suffix of BLOCKED_HOME_SUBPATHS) aliases.push(resolveBlockedHostPathAlias(path.posix.join(home, suffix)));
	}
	aliases.push(resolveBlockedHostPathAlias(stateDir));
	const cacheKey = aliases.flatMap(({ lexical, canonical }) => [lexical, canonical]).join("\0");
	if (cachedBlockedHostPaths?.key === cacheKey) return cachedBlockedHostPaths.paths;
	const blocked = /* @__PURE__ */ new Set();
	for (const alias of aliases) addBlockedHostPath(blocked, alias);
	const paths = [...blocked];
	cachedBlockedHostPaths = {
		key: cacheKey,
		paths
	};
	return paths;
}
function resolveBlockedHostPathAlias(candidate) {
...
function addBlockedHostPath(blocked, alias) {
	blocked.add(alias.lexical);
	if (alias.canonical !== alias.lexical) blocked.add(alias.canonical);
}
