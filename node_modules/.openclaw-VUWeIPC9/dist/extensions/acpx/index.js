...
import { an as unregisterAcpRuntimeBackend, gn as AcpRuntimeError, rn as registerAcpRuntimeBackend } from "../../pi-embedded-iRgRpYxO.js";
import { a as resolveWindowsSpawnProgramCandidate, n as materializeWindowsSpawnProgram, t as applyWindowsSpawnProgramPolicy } from "../../windows-spawn-BMCppuAI.js";
import "../../core-C34-w2tw.js";
import "../../extension-shared-A-QvwGJl.js";
import { t as zod_exports } from "../../zod-DjFifMg3.js";
import "../../runtime-api-D6LMr5Vg.js";
...
var AcpxRuntime = class {
	...
	async shouldReplaceEnsuredSession(params) {
		...
		if (status === "dead") {
			...
			if (shouldRetainNamedSessionForDeadStatus(detail)) {
				this.logger?.warn?.(`acpx ensureSession retaining dead named session with recoverable status: session=${params.sessionName} cwd=${params.cwd} status=${status} summary=${summary || "<empty>"}`);
				return false;
			}
	...
	async recoverEnsureFailure(params) {
		...
		if (status === "dead") {
			const summary = summarizeLogText(asOptionalString(detail?.summary) ?? "");
			if (shouldRetainNamedSessionForDeadStatus(detail)) {
				this.logger?.warn?.(`acpx ensureSession retaining dead named session after ensure failure with recoverable status: session=${params.sessionName} cwd=${params.cwd} status=${status} summary=${summary || "<empty>"}`);
				return events;
			}
