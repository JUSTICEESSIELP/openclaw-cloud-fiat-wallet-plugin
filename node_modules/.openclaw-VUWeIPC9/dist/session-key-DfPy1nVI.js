...
function normalizeThreadSuffixChannelHint(value) {
	return (value ?? "").trim().toLowerCase() || void 0;
}
function inferThreadSuffixChannelHint(sessionKey) {
	const parts = sessionKey.split(":").filter(Boolean);
	if (parts.length === 0) return;
	if ((parts[0] ?? "").trim().toLowerCase() === "agent") return normalizeThreadSuffixChannelHint(parts[2]);
	return normalizeThreadSuffixChannelHint(parts[0]);
}
function parseThreadSessionSuffix(sessionKey, options) {
	const raw = (sessionKey ?? "").trim();
	if (!raw) return {
	...
	return {
		...
		threadId: (markerIndex === -1 ? void 0 : raw.slice(markerIndex + marker.length))?.trim() || void 0
	};
}
function parseSessionConversationRef(sessionKey) {
	const raw = (sessionKey ?? "").trim();
	if (!raw) return null;
	...
	const { baseSessionKey, threadId } = parseThreadSessionSuffix(joined, { channelHint: channel });
