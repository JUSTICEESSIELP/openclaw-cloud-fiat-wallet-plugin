...
import { $t as readAcpSessionEntry, ct as resolveTaskForLookupToken, it as listTaskRecords, lt as setTaskCleanupAfterById, nt as ensureTaskRegistryReady, ot as markTaskLostById, rt as getTaskById, st as maybeDeliverTaskTerminalUpdate, tt as deleteTaskRecordById } from "./pi-embedded-iRgRpYxO.js";
...
function runTaskRegistryMaintenance() {
	ensureTaskRegistryReady();
	const now = Date.now();
	let reconciled = 0;
	let cleanupStamped = 0;
	...
	for (const task of listTaskRecords()) {
		if (shouldMarkLost(task, now)) {
			if (markTaskLost(task, now).status === "lost") reconciled += 1;
			continue;
		}
		...
		if (shouldStampCleanupAfter(task) && setTaskCleanupAfterById({
			taskId: task.taskId,
